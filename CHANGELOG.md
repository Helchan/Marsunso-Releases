...
## [2.0.1] - 2026-02-14
修复后台报错问题：把内联 onerror 改为用 addEventListener 绑定避免策略禁止内联事件处理器的问题
...
#### 检测功能支持中途停止
在批量检测（目录检测、全量检测）过程中，用户现可中途停止检测：

- **新增停止按钮**：检测中显示红色“停止”按钮
- **交互状态**：
  - 检测中：停止按钮可点击
  - 点击停止后：按钮置灰显示“停止中...”，等待当前请求完成
  - 检测结束后：显示“确定”按钮
- **结果展示**：停止后显示已检测的统计结果（已检测 X/N 个书签）

### 影响文件

| 文件 | 变更类型 | 说明 |
|------|---------|------|
| `manifest.json` | 修改 | 版本号更新：2.0.0 → 2.0.1 |
| `popup.html` | 修改 | 提示对话框新增“停止”按钮 |
| `popup.css` | 修改 | 新增停止按钮样式（红色、禁用灰色） |
| `popup.js` | 修改 | 新增取消标志、停止按钮事件、并发控制器取消支持 |
...
#### 书签 URL 有效性检测
本次重大版本更新新增了书签有效性检测功能，帮助用户及时发现失效书签，保持书签库的健康状态。

##### 检测状态

| 状态 | 圆点颜色 | 判定条件 |
|------|---------|----------|
| **有效** | 🟢 绿色 | HTTP 状态码 200-299 |
| **失效** | 🔴 红色 | HTTP 状态码 404 |
| **存疑** | 🟡 黄色 | 其他状态码、网络错误、超时等 |
| **未检测** | 无圆点 | 尚未执行检测 |
...
- **交互流程**：
  1. 显示"检测中..."提示弹窗
  2. 后台发起 HTTP HEAD/GET 请求（15秒超时）
  3. 检测完成后更新状态圆点
  4. 提示切换为"检测完成"，200ms 后自动关闭

##### 目录批量检测
- **功能入口**：选中目录 → 右键 → 点击"检测"
- **交互特性**：
  - 递归检测目录下所有层级的书签（包括子目录）
  - 显示实时进度："检测中 0/N..."
  - 并发控制：最大 5 个并发请求
  - 完成后显示统计结果：共 N 个书签，🟢 X  🟡 Y  🔴 Z

##### 目录状态聚合
目录本身会显示状态圆点，表示该目录下所有书签的整体健康状况：
...
|----------|-------------|----------|
| 默认 | 灰色 | 显示全部书签（含目录） |
| 绿色 | 🟢 #10b981 | 仅显示**有效**书签和目录 |
| 黄色 | 🟡 #eab308 | 仅显示**存疑**书签（不含目录） |
| 红色 | 🔴 #ef4444 | 仅显示**失效**书签（不含目录） |
...
##### 检测机制
- 检测请求在 Background Service Worker 中执行，绕过 Popup 的 CORS 限制
- 使用 `chrome.runtime.sendMessage` 进行 Popup ↔ Background 通信
- 同时发起 HEAD 和 GET 请求，任一成功即返回
- 检测结果存储在 `chrome.storage.local`，键名 `bookmarkValidationStatus`

##### 特殊 URL 处理
以下 URL 直接标记为"存疑"，不发起检测请求：
- `javascript:` 伪协议
- `chrome://` / `chrome-extension://` 浏览器内部页面
- `file://` 本地文件
- `data:` / `about:` / `edge:` 特殊协议

### 影响文件

| 文件 | 变更类型 | 说明 |
|------|---------|------|
| `manifest.json` | 修改 | 版本号更新：1.2.8 → 2.0.0 |
| `background.js` | 修改 | 新增 URL 检测相关消息处理、检测算法、结果存储 |
| `popup.js` | 修改 | 新增检测交互、状态圆点更新、搜索筛选功能 |
| `popup.css` | 修改 | 新增状态圆点样式、搜索图标颜色状态样式 |
| `popup.html` | 修改 | 右键菜单新增"检测"菜单项 |
...
- ✅ **书签编辑功能**：已实现（v1.1.7）
- ✅ **书签失效检测**：已实现（v2.0.0）- 检测 URL 可访问性，状态圆点显示，搜索面板状态筛选
